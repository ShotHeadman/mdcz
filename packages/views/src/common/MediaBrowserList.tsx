      ...
      <ContextMenuTrigger asChild>
        ...
        >
          {item.selectionControl ? (
            <div className="flex shrink-0 items-center px-4 py-4">{item.selectionControl}</div>
          ) : null}
          <button
            type="button"
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              {item.subtitle ? (
                <div className="mt-1 truncate text-[13px] text-muted-foreground">{item.subtitle}</div>
              ) : null}
              {item.errorText ? <div className="mt-1.5 truncate text-xs text-destructive">{item.errorText}</div> : null}
            </div>
