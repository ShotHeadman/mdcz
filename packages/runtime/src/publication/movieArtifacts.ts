...
> => {
  ...
  for (const file of input.members) {
    ...
    if (layout.mode === "move" && resolve(sourcePath) !== resolve(layout.targetVideoPath)) {
      ...
      if (mediaContent !== undefined) {
        move.rewrittenContent = mediaContent;
        size = Buffer.byteLength(mediaContent);
        modifiedAt = new Date();
      }
      moves.push(move);
    ...
    for (const sidecar of layout.sidecars) {
      ...
      if (sidecar.kind === "subtitle") {
        ...
        if (mediaSources.has(resolve(sidecar.sourcePath)) || featureAssets.has(sidecar.targetPath)) continue;
        featureAssets.set(sidecar.targetPath, {
          type: "local",
          kind: "feature",
        ...
        if (moving) {
          ...
          moves.push({
            ...
            dev: sidecarSource.dev,
            ino: sidecarSource.ino,
          });
          publishedTargets.push(target);
        }
      }
  ...
  for (const [metadataOutputDir, file] of locations) {
    ...
    const groups = [
      {
        key: "thumb" as const,
        paths: [input.assetDecisions?.thumb === "replace" ? downloaded.thumb : (downloaded.thumb ?? existing?.thumb)],
      },
      {
        key: "poster" as const,
        paths: [
          input.assetDecisions?.poster === "replace" ? downloaded.poster : (downloaded.poster ?? existing?.poster),
        ],
      },
      {
        key: "fanart" as const,
        paths: [
          input.assetDecisions?.fanart === "replace" ? downloaded.fanart : (downloaded.fanart ?? existing?.fanart),
        ],
      },
      {
        ...
        if (!targetPath) {
          ...
          if (stagedName || sourcePath !== targetPath) {
            ...
            artifacts.push({
              ...
              removeSourcesAfterCommit: input.relocateExistingArtifacts && !stagedName ? [sourcePath] : undefined,
            });
            publishedTargets.push(target);
  ...
  let nfoPath = await input.writeNfo({ ...assets, downloaded: [...downloadedTargets] }, async (targetPath, data) => {
