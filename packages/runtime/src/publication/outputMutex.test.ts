...
describe("output directory serialization", () => {
  it("queues intersecting directory sets, leaves independent outputs free, and releases once", async () => {
    ...
    await expect(
      acquireOutputDirectories(["/a/error.mp4"], async () => {
        throw new Error("directory unavailable");
      }),
    ).rejects.toThrow("directory unavailable");
    const failedTask = async () => {
      const unlock = await acquireOutputDirectories(["/a/error.mp4"]);
      try {
    ...
    await expect(failedTask()).rejects.toThrow("publication failed");
    (await acquireOutputDirectories(["/a/recovered.mp4"]))();
  });
});
