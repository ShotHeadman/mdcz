...
const executionFor = (): ScrapeRunExecution<ScrapeRunItem, string> => {
  ...
  return {
    movieGroups: [
      { movieId: "m1", members: [items[0], items[1]], assets: [] },
      { movieId: "m2", members: [items[2]], assets: [] },
    ],
    concurrency: 1,
    prepareGroup: async (group) => ({ status: "prepared", prepared: group.members.map((item) => item.id).join(",") }),
    checkTargets: async () => undefined,
    executePreparedGroup: async ({ group }) => ({
      results: group.members.map((item) => ({ itemId: item.id, result: resultFor(item) })),
    }),
...
describe("scrape movie groups", () => {
  it("retains whole-group preparation across pause and resume", async () => {
    ...
    expect(execute.mock.calls.map(([entry]) => entry.group.members.map((item) => item.id))).toEqual([
      ["one", "two"],
      ["independent"],
    ]);
  ...
  it.each(["stop", "interrupt"] as const)("settles every member when a preparing movie is %s", async (action) => {
    const execution = executionFor();
    const started = deferred();
    execution.prepareGroup = async (group, signal) => {
      started.resolve();
      await new Promise<void>((done) => signal.addEventListener("abort", () => done(), { once: true }));
      return { status: "skipped", result: resultFor(group.members[0], "skipped") };
    };
    const session = new ScrapeRunSession({
      runId: `group-${action}`,
      totalItems: 3,
