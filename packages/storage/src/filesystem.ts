  ...
  for (const entry of entries) {
    ...
    try {
      if (entry.isDirectory()) {
        if (recursive) {
          files.push(...(await walkRootDirectory(root, entryRelativePath, true, visitedDirs)));
        }
        continue;
      }
...
export const listRootFiles = async (
  ...
  try {
    return await walkRootDirectory(root, normalizedRelativePath, recursive, new Set<string>());
  } catch (error) {
    throw toStorageError(error, relativePath);
  }
};
